...
_VALUE_COLS = [
    ...
    ("LILATracts_1And10", _FOOD, "Low-income & low-access tract (1/10 mi)", "Share of the residential buffer in tracts flagged low-income and low-access at 1 mile (urban) / 10 miles (rural)."),
    ("LATracts1", _FOOD, "Low-access tract (1 mi)", "Share of the residential buffer in tracts flagged low-access to a supermarket at 1 mile."),
    ("HUNVFlag", _FOOD, "Low-vehicle-access tract", "Share of the residential buffer in tracts where >=100 households have no vehicle and are beyond half a mile from a supermarket."),
    ("LowIncomeTracts", _FOOD, "Low-income tract", "Share of the residential buffer in tracts meeting the USDA low-income criterion (poverty rate >=20% or median family income below the area threshold)."),
    # ndi
    ("ndi", _NDI, "Neighborhood Deprivation Index score", "Neighborhood Deprivation Index — composite socioeconomic deprivation score (z-score)."),
    # walkability
...
# No statistical-type column in the dictionary: every exported feature is
# continuous, so the field would be a constant. Even the four USDA tract flags
# arrive continuous — their 0/1 source is averaged over the residential buffer,
# so a buffer straddling a tract boundary exports the share of its area in
# flagged tracts (measured: 7 of 100 demo patients, every one of them
# boundary-straddling). Should a genuinely two-valued column ever ship, that is
# the point to add the field back.
...
def write_feature_dictionary(out_path: Path) -> int:
    """Emit the per-column table the backend serves at runtime.

    The ontology lives under frontend/public/, which the backend image does
    not contain, so the same authored table is materialized here as data the
    container ships with. Definitions drop the "(Result column: X.)" suffix
    the ontology adds — the dictionary already has a column-name field.
    """
    entries = {
        col: {"label": label, "definition": definition}
        for col, _parent, label, definition in _VALUE_COLS
    }
    payload = {
        "_comment": (
            "Per-column feature dictionary, generated by "
            "scripts/extend_ontology.py from the same table that authors the "
            "ontology value_col nodes. Do not hand-edit: rerun the script."
        ),
...
if __name__ == "__main__":
    ...
    default_dict = Path(__file__).resolve().parents[1] / "app" / "data" / "feature_dictionary.json"
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--ontology-dir", default=str(default_dir))
    parser.add_argument("--dictionary-out", default=str(default_dict))
