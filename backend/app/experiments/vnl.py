"""Single-experiment orchestrator: BG-tagged VIIRS Night-time Lights (VNL).

Spawned by app.dispatcher as:
    python -m app.experiments.vnl run <task_dir> [--variables vnl]

Cloned from noise.py with the spec-mandated Sprint 10 deltas:
  * _BOUNDARY = 'BG_VNL' (avoids collision with BG / BG_TIGER / BG_NHD /
    BG_NOISE / BG_TEMIS caches)
  * C3 step c3_vnl -> c3/vnl_grid_demo.yaml (linkage_pattern=grid_weights,
    produces a per-cohort grid_id->weight table)
  * C4 step c4_vnl -> c4/vnl_demo.yaml (linkage_pattern=gridded,
    plugin=vnl; reads per-cohort weights AND annual VNL TIFs)
...
_C3_STEP = PipelineStep(
    name="c3_vnl",
    template_relpath="c3/vnl_grid_demo.yaml",
    is_c3=True,
)
...
_VARIABLE_TO_STEP = {
    "vnl": PipelineStep(
        name="c4_vnl",
        template_relpath="c4/vnl_demo.yaml",
        is_c3=False,
    ),
}

_PARQUET_MAP = {"vnl": "c4_vnl.parquet"}
...
def _sanity_check_pipeline_supports_gridded_episode() -> None:
    ...
    if "output_grouping" not in src:
        raise RuntimeError(
            "vnl: live spacescans.linkage.gridded_linkage does not mention "
...
def plan(config: dict) -> list[PipelineStep]:
    """Always emits [c3_vnl, c4_vnl].

    vnl has a single variable with a single value_col emitted by a single
    ...
    return [_C3_STEP, _VARIABLE_TO_STEP["vnl"]]
...
def render_yaml(step: PipelineStep, task_dir: Path, user_config: dict) -> Path:
    ...
    Two structural divergences from bg_ndi_wi.render_yaml / zcta5_cbp.render_yaml:
      1. No raster_res_m write — VNL TIFs are precomputed at a fixed grid.
      2. On the C4 step only, rewrite cfg['source']['file'] (NOT exposure.file)
         ...
         the weights table). cfg['exposure']['file'] (the VNL C3 directory)
         stays untouched — pipeline CLI --data-dir resolves it against
         SPACESCANS_DATA_DIR.
    """
    template_path = (
    ...
    # NOTE: no raster_res_m write (VNL TIFs are precomputed at fixed grid).

    if step.is_c3:
        # C3: pipeline CLI --data-dir resolves source.file (the 2013 VNL TIF);
        ...
        # C3 output. cfg['exposure']['file'] (the VNL C3 directory) is left
        # alone — pipeline CLI --data-dir resolves it.
        if not isinstance(cfg.get("source"), dict):
            raise RuntimeError(
                "vnl.render_yaml: unexpected source: shape in C4 template"
            )
        cfg["source"]["file"] = str(
...
def merge_results(task_dir: Path, variables: list[str]) -> Path:
    ...
    _PARQUET_MAP has a single entry (vnl -> one parquet); the merge picks
    up the single value_col from variable_registry.get_variable.
    """
...
def run(task_dir: Path, variables: list[str] | None = None) -> int:
    """Main entry point. Mirrors noise.run with an override `variables`."""
...
def _cli_main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="vnl")
