"""Sprint 10 T3: temis runner — mirror of test_vnl.py / test_noise.py.
...
tiger_proximity AND nhd_bluespace AND noise AND vnl), the gridded sanity
probe, and merge_results delegation into _merge.write_partial.
"""
...
from app.experiments import temis


@pytest.fixture()
def task_dir(tmp_path: Path) -> Path:
    d = tmp_path / "task_temis_T3"
    d.mkdir()
...
def templates_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Stub C3 + C4 templates. The C3 temis template intentionally has no
    ...
    by render_yaml) and `exposure` (the TEMIS C4 raw directory — left
    ...
    (templates / "c4" / "temis_demo.yaml").write_text(yaml.safe_dump({
        ...
        "buffer": {"patient_file": "PLACEHOLDER", "buffer_m": 270},
        "source": {
            "file": ("output/python_v2/270m/TEMIS/C3/"
...
def test_plan_rejects_empty_variables() -> None:
    with pytest.raises(ValueError, match="at least one variable"):
        temis.plan({"variables": []})


def test_plan_rejects_unknown_variable() -> None:
    with pytest.raises(ValueError, match="unknown variable"):
        temis.plan({"variables": ["ndi"]})  # ndi belongs to bg_ndi_wi


def test_plan_returns_c3_then_c4_order(task_dir: Path) -> None:
    config = json.loads((task_dir / "config.json").read_text())
    steps = temis.plan(config)
    assert [s.name for s in steps] == ["c3_temis", "c4_temis"]
...
def test_render_yaml_c3_leaves_source_untouched(
    ...
    """C3 template's source.file (the TEMIS template raster) must NOT be
    rewritten — pipeline CLI --data-dir resolves it against
    SPACESCANS_DATA_DIR.
    """
    cfg = {"buffer": {"size": 270}}
    c3_path = temis.render_yaml(temis._C3_STEP, task_dir, cfg)
...
def test_render_yaml_c4_rewrites_source_to_per_task_c3_parquet(
    ...
    Sprint 9 noise / Sprint 10 vnl pattern — divergent from TIGER/NHD which
    rewrite exposure.file).
    """
    cfg = {"buffer": {"size": 270}}
    c4_step = temis._VARIABLE_TO_STEP["temis"]
    c4_path = temis.render_yaml(c4_step, task_dir, cfg)
    rendered = yaml.safe_load(c4_path.read_text())
    expected = str(task_dir / "output" / "c3_temis.parquet")
...
def test_render_yaml_c4_leaves_exposure_file_untouched(
    ...
    assert "TEMIS/C4/raw" in rendered["exposure"]["file"]
...
def test_render_yaml_injects_output_grouping_episode(
    ...
    cfg = {"buffer": {"size": 270}}
    c4_path = temis.render_yaml(
        temis._VARIABLE_TO_STEP["temis"], task_dir, cfg
    )
    rendered = yaml.safe_load(c4_path.read_text())
...
def test_render_yaml_does_not_inject_raster_res_m(
    ...
    """TEMIS template grid is precomputed — render_yaml must not invent a
    ...
    for step in (temis._C3_STEP, temis._VARIABLE_TO_STEP["temis"]):
        path = temis.render_yaml(step, task_dir, cfg)
...
def test_cache_key_differs_from_other_experiments_in_boundary(
    ...
    """R1 lock: temis, vnl, noise, nhd_bluespace, tiger_proximity, bg_ndi_wi
    ...
    """
    from app.experiments import bg_ndi_wi, nhd_bluespace, tiger_proximity, noise, vnl
    ...
    # temis shares 3-segment shape with vnl / noise / NHD / TIGER.
    assert len(temis_key.split("__")) == 3
    # All six keys mutually distinct.
    assert len({temis_key, vnl_key, noise_key, nhd_key, tiger_key, bg_key}) == 6
...
def test_merge_results_delegates_to_write_partial(
    ...
    temis-specific experiment_key and parquet_map."""
    ...
    }).to_parquet(out_dir / "c4_temis.parquet", index=False)
    ...
    out = temis.merge_results(task_dir, variables=["temis"])
    assert out == out_dir / "result_temis.csv"
    assert captured["experiment_key"] == "temis"
    assert captured["variables"] == ["temis"]
    assert captured["parquet_map"] == {"temis": "c4_temis.parquet"}
...
def test_sanity_check_passes_against_current_pipeline():
    ...
    temis._sanity_check_pipeline_supports_gridded_episode()
